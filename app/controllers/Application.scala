...
import models.{UserQuery, UsersQuiries}
...
//
  ...
  def getData(id:Long) = Action {
    implicit request =>
      val q = table.filter {_.id === id}
      Await.result(dbConfig.db.run(q.result).map {
        answ => {
          try {
            val oneRow = answ.take(1)
            Ok(Json.toJson(Map("answ" -> getInfo(oneRow.head.siteUrl, oneRow.head.htmlTags))).toString())
          }
          catch {
            case urlExp: IllegalArgumentException => BadRequest("Incorrect url")
            case e: UnknownHostException => BadRequest("Error while parsing. Host is unreachable")
